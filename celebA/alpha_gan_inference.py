...
def get_sequential_batch(file_iter,num_images):
    #random_file_iter = np.random.choice(file_iter,batch_size,replace=False);
    random_file_iter = file_iter[0:num_images];
    #print("Random_file_iter");
    #print(random_file_iter);
    #sys.exit(0);
    X = np.zeros([len(random_file_iter),opts['img_height'],opts['img_width'],opts['num_channels']]);
...
def get_sequential_batch_for_recons(file_iter,num_images,start_image_number,stop_image_number):
    #random_file_iter = np.random.choice(file_iter,batch_size,replace=False);
    random_file_iter = file_iter[start_image_number:stop_image_number];
    #print("Random_file_iter");
    #print(random_file_iter);
    #sys.exit(0);
    X = np.zeros([len(random_file_iter),opts['img_height'],opts['img_width'],opts['num_channels']]);
...
def generate(model_number):
    with tf.Session() as sess:
        ...
        val_files = range(val_min_file_num, 1+val_max_file_num);
        val_file_iter=[os.path.join(celeb_source, '%s' % str(i).zfill(6)) for i in val_files];
        #output_directory = opts['original_crop_dir'];
        if not os.path.exists(output_directory):
            os.makedirs(output_directory);
        num_val_img = 25;
        batch_X = get_sequential_batch(val_file_iter,num_val_img);
        ...
        plt.axis('off');
        plt.imshow(original, origin="upper",interpolation='nearest', cmap="gray");
        plt.savefig('orig-img_'+str(model_number)+'.png');
        plt.close();
        ...
        plt.axis('off');
        plt.imshow(reconstructed, origin="upper",interpolation='nearest', cmap="gray");
        plt.savefig('recons-img_'+str(model_number)+'.png');
        plt.close();
        ...
        plt.axis('off');
        plt.imshow(generated_images, origin="upper",interpolation='nearest', cmap="gray");
        plt.savefig('gen-img_'+str(model_number)+'.png');
        plt.close();

def generateImages(num_images,model_number,n,output_directory):
    with tf.Session() as sess:
        sess.run(tf.global_variables_initializer());
...
def generateReconstructedImages(num_images,model_number=15,n=1):
    with tf.Session() as sess:
        sess.run(tf.global_variables_initializer());
        ...
        for batch in range(n_batches):
            ...
            batch_X = get_sequential_batch_for_recons(val_file_iter,n,start_image_number,stop_image_number);
...
# models = [10];
generateReconstructedImages(num_images=10000,model_number=10,n=100);
# generate(model_number=models[0]);
# for model_num in models:
#     numSets = 5;
#     for i in range(1,numSets+1):
#         generateImages(num_images=10000,model_number=model_num,n=100,output_directory='alpha_gan_'+str(model_num)+'/alpha_gan_generations_'+str(i)+'/');
#train();
